...
### 4.1 runtime-service 开发挂载

如果只在本地开发 `apps/runtime-service` 的源码、prompt、skills 或测试辅助文件，可以叠加开发 override：

```bash
docker compose -f deploy/docker-compose.stack.yml -f deploy/docker-compose.stack.dev.yml --env-file deploy/.env.stack up -d runtime-service
```

该 override 会把宿主机源码挂载到容器内 `/deps/runtime-service`：

```text
../apps/runtime-service -> /deps/runtime-service
```

这样修改 `apps/runtime-service/runtime_service/**` 后，容器内可直接看到新文件。若 Python 进程未自动 reload，重启服务即可，无需重新 build：

```bash
docker compose -f deploy/docker-compose.stack.yml -f deploy/docker-compose.stack.dev.yml --env-file deploy/.env.stack restart runtime-service
```

依赖、Dockerfile 或基础镜像变化仍需按标准流程 rebuild。
