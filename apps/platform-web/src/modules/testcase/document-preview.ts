...
export function resolveDocumentStoragePath(document: PreviewDocumentLike | null): string {
  if (!document) {
    return ''
  }
...
export function resolveDocumentContentType(document: PreviewDocumentLike | null): string {
  if (!document) {
    return ''
  }
