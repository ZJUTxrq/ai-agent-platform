...
def test_multimodal_middleware_sanitizes_stale_attachment_blocks() -> None:
    def fake_parser(
        artifact: AttachmentArtifact, block: Mapping[str, Any]
    ) -> AttachmentArtifact:
        del block
        ...
        next_artifact["status"] = "parsed"
        next_artifact["summary_for_model"] = f"PDF 已解析：{artifact['name']}"
        next_artifact["structured_data"] = {"key_points": [str(artifact["name"])]}
        return cast(AttachmentArtifact, next_artifact)
    ...
    def handler(updated_request: ModelRequest) -> ModelResponse:
        for message in updated_request.messages:
            ...
            if not isinstance(content, list):
                continue
            assert all(
                not (isinstance(item, dict) and item.get("type") in {"image", "file"})
                for item in content
            )
        first_content = cast(list[dict[str, Any]], updated_request.messages[0].content)
        second_content = cast(list[dict[str, Any]], updated_request.messages[2].content)
        assert first_content[1]["type"] == "text"
        assert "原始附件内容已从后续模型请求中移除" in first_content[1]["text"]
        assert second_content[1]["type"] == "text"
        assert "PDF 已解析：b.pdf" in second_content[1]["text"]
        return ModelResponse(result=[AIMessage(content="ok")])
