...
MULTIMODAL_SOURCE_BLOCKS_KEY = _types.MULTIMODAL_SOURCE_BLOCKS_KEY
...
class MultimodalMiddleware(AgentMiddleware[AgentState[Any], Any]):
    ...
    @staticmethod
    def _read_state_source_blocks(
        state: Mapping[str, Any] | None,
    ) -> dict[str, dict[str, Any]]:
        if state is None:
            return {}
        value = state.get(MULTIMODAL_SOURCE_BLOCKS_KEY)
        if not isinstance(value, Mapping):
            return {}
        blocks: dict[str, dict[str, Any]] = {}
        for key, item in value.items():
            if isinstance(key, str) and key.strip() and isinstance(item, Mapping):
                blocks[key] = dict(item)
        return blocks
    ...
    def _merge_source_blocks(
        ...
        blocks = MultimodalMiddleware._read_state_source_blocks(state)
        for artifact, block in current_pairs:
            fingerprint = MultimodalMiddleware._artifact_fingerprint(artifact)
            if fingerprint is None or fingerprint in blocks:
                continue
            blocks[fingerprint] = dict(block)
        return blocks
    ...
    def _prepare_artifact_parsing(
        ...
        source_blocks = self._merge_source_blocks(state, current_pairs)
        if source_blocks:
            state[MULTIMODAL_SOURCE_BLOCKS_KEY] = source_blocks
        else:
            state.pop(MULTIMODAL_SOURCE_BLOCKS_KEY, None)
