...
from .protocol import (
    ...
    _resolve_attachment_fingerprint,
    _resolve_attachment_name,
)
from .types import (
...
def _rewrite_latest_human_message_for_model(
    ...
    *,
    ...
    return _sanitize_attachment_blocks_for_model(
        rewritten_messages,
        artifacts,
        include_parsed_text=include_parsed_text,
        parsed_text_max_chars=parsed_text_max_chars,
...
def _fallback_attachment_block_text(item: Mapping[str, Any]) -> str:
    name = _resolve_attachment_name(item) or "未命名附件"
    block_type = item.get("type") or "file"
    return (
        f"[附件: {name}; type={block_type}; status=removed]\n"
        "原始附件内容已从后续模型请求中移除；请参考系统提示中的多模态附件摘要。"
    )


def _sanitize_attachment_blocks_for_model(
    messages: Sequence[Any],
    artifacts: Sequence[AttachmentArtifact],
    *,
    include_parsed_text: bool,
    ...
    artifacts_by_fingerprint = {
        fingerprint: artifact
        for artifact in artifacts
        if isinstance((provenance := artifact.get("provenance")), Mapping)
        and isinstance((fingerprint := provenance.get("fingerprint")), str)
    ...
    for message in messages:
        ...
        if not isinstance(content, list) or not any(
            _is_attachment_block(item) for item in content
        ):
        ...
        for item in content:
            ...
            if isinstance(item, Mapping):
                fingerprint = _resolve_attachment_fingerprint(item)
                artifact = (
                    artifacts_by_fingerprint.get(fingerprint)
            ...
            sanitized_content.append(
                {
                    "type": "text",
                    "text": (
                        _artifact_model_text_with_options(
                            artifact,
                            include_parsed_text=include_parsed_text,
                            parsed_text_max_chars=parsed_text_max_chars,
                        )
                        if artifact is not None
                        else _fallback_attachment_block_text(item)
                    ),
                }
            )

        sanitized_messages.append(
            _copy_message_with_content(message, sanitized_content)
            if changed
            else message
        )
