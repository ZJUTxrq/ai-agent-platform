...
from runtime_service.services.test_case_service_v2.knowledge_mcp import (
...
from runtime_service.services.test_case_service_v2.middleware import (
...
from runtime_service.services.test_case_service_v2.tool_runtime_context_middleware import (
    ToolRuntimeContextSanitizerMiddleware,
)
from runtime_service.services.test_case_service_v2.tool_argument_alias_middleware import (
    ToolArgumentAliasMiddleware,
)
from runtime_service.services.test_case_service_v2.tools import (
...
def _resolve_service_config_for_run() -> TestCaseServiceConfig:
    try:
        config = get_config()
    except RuntimeError:
        return TEST_CASE_SERVICE_CONFIG
...
def _resolve_required_tools(_settings: ResolvedRuntimeSettings) -> list[Any]:
    ...
    return [
        *get_test_case_knowledge_tools(service_config),
        *build_test_case_service_tools(service_config),
    ]
...
async def _aresolve_required_tools(_settings: ResolvedRuntimeSettings) -> list[Any]:
    service_config = _resolve_service_config_for_run()
    return [
        *(await aget_test_case_knowledge_tools(service_config)),
        *build_test_case_service_tools(service_config),
    ]
