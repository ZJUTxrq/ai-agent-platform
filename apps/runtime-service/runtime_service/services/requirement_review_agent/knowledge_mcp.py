...
def _build_lazy_knowledge_tool(
    ...
    async def call_tool(**kwargs: Any) -> str:
        ...
        except Exception as exc:  # noqa: BLE001 - 连接失败不应中断评审流程
            logger.warning(
                "requirement_review_agent knowledge MCP call failed",
                extra={
            ...
            return json.dumps(
                {
                    ...
                    "hint": (
                        "知识库当前不可用。请按 skill 要求明确声明知识库证据基线为空，"
                        "不要虚构查询结果。"
                    ),
                },
                ensure_ascii=False,
...
async def aget_requirement_review_knowledge_tools(
    ...
    return get_requirement_review_knowledge_tools(service_config)
        ...
        return json.dumps(
            {
                ...
                "detail": body[:500],
                "hint": (
                    "知识库工具执行失败。请按 skill 要求明确声明知识库证据基线为空，"
                    "不要虚构查询结果。"
                ),
            },
            ensure_ascii=False,
...
__all__ = [
    ...
    "build_requirement_review_knowledge_mcp_specs",
