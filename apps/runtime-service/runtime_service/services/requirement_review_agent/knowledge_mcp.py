...
REQUIREMENT_REVIEW_KNOWLEDGE_SERVER_NAME = "requirement_review_knowledge"
...
def build_requirement_review_knowledge_mcp_specs(
    ...
    return {
        REQUIREMENT_REVIEW_KNOWLEDGE_SERVER_NAME: {
            "transport": "sse",
            "url": url,
...
async def aget_requirement_review_knowledge_tools(
    service_config: RequirementReviewAgentConfig,
) -> list[Any]:
    if not service_config.knowledge_mcp_enabled:
        return []

    specs = build_requirement_review_knowledge_mcp_specs(service_config)
    if not specs:
        return []
    ...
    try:
        ...
        _validate_required_knowledge_tools(tools)
    except Exception:
        logger.warning(
            "requirement_review_agent knowledge MCP unavailable, fallback to local-only tools",
            extra={
                "knowledge_mcp_url": service_config.knowledge_mcp_url,
                "knowledge_timeout_seconds": service_config.knowledge_timeout_seconds,
                "knowledge_sse_read_timeout_seconds": (
                    service_config.knowledge_sse_read_timeout_seconds
                ),
            },
            exc_info=True,
        )
...
def get_requirement_review_knowledge_tools(
    service_config: RequirementReviewAgentConfig,
) -> list[Any]:
    return asyncio.run(aget_requirement_review_knowledge_tools(service_config))
...
def _validate_required_knowledge_tools(tools: list[Any]) -> None:
    available_names = {
        str(getattr(tool, "name", "") or "").strip()
        for tool in tools
        if str(getattr(tool, "name", "") or "").strip()
    }
    missing_names = sorted(REQUIRED_KNOWLEDGE_TOOL_NAMES - available_names)
    if missing_names:
        raise ValueError(
            "missing_required_knowledge_tools:" + ",".join(missing_names)
        )
