...
class RequirementReviewDocumentPersistenceMiddleware(
    AgentMiddleware[MultimodalAgentState, Any]
):
    state_schema = MultimodalAgentState

    def __init__(self, service_config: RequirementReviewAgentConfig) -> None:
    ...
    def wrap_model_call(
        ...
        if not service_config.persistence_enabled or not self._should_persist(request):
    ...
    async def awrap_model_call(
        ...
        outcome = await apersist_requirement_review_documents(
            runtime=request.runtime,
            state=request.state if isinstance(request.state, Mapping) else {},
            service_config=service_config,
            messages=list(request.messages),
...
__all__ = ["RequirementReviewDocumentPersistenceMiddleware"]
