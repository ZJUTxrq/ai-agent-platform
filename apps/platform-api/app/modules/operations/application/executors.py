...
class RequirementReviewDocumentsExportExecutor(OperationExecutorProtocol):
    ...
    async def execute(
        ...
        *,
        ...
        project_id = _clean(operation.project_id)
        if not project_id:
            raise ValueError("project_id is required for requirement review documents export")
...
class RequirementReviewResultsExportExecutor(OperationExecutorProtocol):
    ...
    async def execute(
        ...
        *,
        ...
        project_id = _clean(operation.project_id)
        if not project_id:
            raise ValueError("project_id is required for requirement review results export")

        filename, media_type, payload = await self._service.export_results(
            actor=actor,
            project_id=project_id,
            query=ExportRequirementReviewResultsQuery(
                batch_id=_clean(operation.input_payload.get("batch_id")),
                quality_gate=_clean(operation.input_payload.get("quality_gate")),
                generation_policy=_clean(operation.input_payload.get("generation_policy")),
                query=_clean(operation.input_payload.get("query")),
            ),
        )
        artifact = self._artifact_store.save_bytes(
