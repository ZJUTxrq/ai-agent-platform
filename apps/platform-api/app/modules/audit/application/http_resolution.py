...
def _looks_like_identifier(segment: str) -> bool:
    ...
    if ":" in normalized and len(normalized) >= 16:
        return True
    if len(normalized) >= 24 and re.fullmatch(r"[A-Za-z0-9_-]+", normalized):
        return True
    return False


def _compress_route_target_id(*, path: str, route_path: str | None) -> str:
    candidate = clean_str(route_path) or path
    if len(candidate) <= _MAX_TARGET_ID_LENGTH:
        return candidate

    segments = _route_segments(path)
    compressed_segments = [
        "{id}" if _looks_like_identifier(segment) else segment
        for segment in segments
    ]
    compressed = "/" + "/".join(compressed_segments)
    if len(compressed) <= _MAX_TARGET_ID_LENGTH:
        return compressed

    # Last-resort guard to honor the storage contract without failing the request path.
    return compressed[: _MAX_TARGET_ID_LENGTH - 3] + "..."
...
def _resolve_action(
    *,
    ...
    return "system.route.requested", "route", _compress_route_target_id(
        path=path,
