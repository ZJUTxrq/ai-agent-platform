...
from app.modules.operations.application.executors import (
    ...
    RequirementReviewDocumentsExportExecutor,
    RequirementReviewResultsExportExecutor,
...
class _FakeRequirementReviewService:
    def __init__(self) -> None:
        self.last_documents_query = None
    ...
    async def export_documents(self, *, actor: ActorContext, project_id: str, query):  # type: ignore[no-untyped-def]
        self.last_documents_query = query
        return (
            "requirement-review-documents.xlsx",
            "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            b"requirement-documents-export",
        )

    async def export_results(self, *, actor: ActorContext, project_id: str, query):  # type: ignore[no-untyped-def]
        self.last_results_query = query
        return (
            "requirement-review-results.xlsx",
            "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            b"requirement-results-export",
        )


class _VisibilityCheckingDispatcher:
    def __init__(self, session_factory) -> None:  # type: ignore[no-untyped-def]
        self.session_factory = session_factory
        self.dispatched_ids: list[str] = []

    async def dispatch(self, *, operation) -> None:  # type: ignore[no-untyped-def]
        with session_scope(self.session_factory) as session:
            repository = SqlAlchemyOperationsRepository(session)
            visible = repository.get_by_id(operation.id)
        if visible is None:
            raise AssertionError("operation_not_committed_before_dispatch")
...
class OperationsArtifactFlowTest(unittest.IsolatedAsyncioTestCase):
    def setUp(self) -> None:
        ...
        self.fake_requirement_review = _FakeRequirementReviewService()
        ...
        self.worker = OperationWorker(
            ...
            executor_registry=OperationExecutorRegistry(
                (
                    ...
                    RequirementReviewDocumentsExportExecutor(
                        service=self.fake_requirement_review,  # type: ignore[arg-type]
                        artifact_store=self.artifact_store,
                    ),
                    RequirementReviewResultsExportExecutor(
                        service=self.fake_requirement_review,  # type: ignore[arg-type]
    ...
    async def test_requirement_review_documents_export_operation_persists_artifact(self) -> None:
        submitted = await self.service.submit_operation(
            actor=self.actor,
            command=CreateOperationCommand(
        ...
        self.assertTrue(processed)
        self.assertEqual(final.status, OperationStatus.SUCCEEDED)
        self.assertEqual(self.fake_requirement_review.last_documents_query.batch_id, "batch-r1")
        self.assertEqual(self.fake_requirement_review.last_documents_query.parse_status, "parsed")
        self.assertEqual(self.fake_requirement_review.last_documents_query.query, "coupon")
        self.assertTrue(final.result_payload["artifact_ready"])
        self.assertEqual(artifact.filename, "requirement-review-documents.xlsx")
        self.assertEqual(artifact.path.read_bytes(), b"requirement-documents-export")

    async def test_requirement_review_results_export_operation_persists_artifact(self) -> None:
        submitted = await self.service.submit_operation(
            actor=self.actor,
            command=CreateOperationCommand(
        ...
        self.assertTrue(processed)
        self.assertEqual(final.status, OperationStatus.SUCCEEDED)
        self.assertEqual(self.fake_requirement_review.last_results_query.batch_id, "batch-r2")
        self.assertEqual(self.fake_requirement_review.last_results_query.quality_gate, "conditional")
        self.assertEqual(
            self.fake_requirement_review.last_results_query.generation_policy,
            "allow_generation_with_assumptions",
        )
        self.assertEqual(self.fake_requirement_review.last_results_query.query, "login")
        self.assertTrue(final.result_payload["artifact_ready"])
        self.assertEqual(artifact.filename, "requirement-review-results.xlsx")
        self.assertEqual(artifact.path.read_bytes(), b"requirement-results-export")

    async def test_submit_operation_dispatches_after_commit(self) -> None:
        dispatcher = _VisibilityCheckingDispatcher(self._session_factory)
        ...
        submitted = await service.submit_operation(
            actor=self.actor,
            command=CreateOperationCommand(
                kind="requirement-review.results.export",
                project_id=self.project_id,
        ...
        self.assertEqual(dispatcher.dispatched_ids, [submitted.id])
